...
Download "wikihowAll.csv" and "wikihowSep.csv" manually from
https://github.com/mahnazkoupaee/WikiHow-Dataset and place them in tfds folder.
...
class Wikihow(tfds.core.GeneratorBasedBuilder):
  ...
  def _generate_examples(self, path=None):
    ...
    with tf.io.gfile.GFile(path) as f:
      reader = csv.reader(f)
      # in WikiAll.txt, keys are headline,title,text.
      # in WikiSep.txt, keys are overview,headline,text,sectionLabel,title.
      key2id = {key: i for i, key in enumerate(next(reader))}
      for i, line in enumerate(reader):
        # skip empty line that only have return token.
        if len(line) > 1:
